...
- `sync_main_to_origin(workspace, repo, remote, *, autostash=True, drop_shas=None) -> str | None`: fetch `<remote> main` and bring local `main` to it (fast-forward, or a surgical reset over divergence), guarding operator WIP with the same stash posture `squash_to_main` uses; return the new HEAD, or `None` when the remote has no `main` (the caller falls back to local HEAD). Over a divergence the reset is **surgical**: unpushed commits are rescued and replayed onto the fresh `origin/main`, except those named in `drop_shas` (the caller's own orphan squash). This preserves an operator cherry-pick that happens to sit on `main` during a land retry. Redundant replays (content already on the new base) collapse to empty picks and are skipped.
