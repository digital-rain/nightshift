...
- Pure modules stay pure: `workflows.py` and `transitions.py` never import the store, git, or HTTP (enforced by `tests/test_import_boundaries.py` — if you touch imports, run it).
...
**Read first:** spec §3 (vocabulary, role resolution), §10 (the two verify definitions); `src/nightshift/_paths.py` (the `asset()` helper); `src/nightshift/manager/scheduler.py:40-72` (agnostic-model + `queue_label`, for role-resolution parity). Nothing else.
...
| `on_workflow_split`, zero children | parent retained, cursor **stays on the split step**, `Progress.INCREMENT` (the existing no-change ladder bounds it) |
...
Visit counting is entry-based (spec §6.4): the *destination* step's counter increments in the same `engine_meta` write that moves the cursor. The first step's visit is counted at first dispatch (Phase 5, not here). Failures never move the cursor, so retries burn no visits by construction — add one test asserting a doc-step `ERROR` transition carries no `engine_meta`.
...
3. **Submit:** compute `WorkflowStepPolicy` from the attempt's work-order workflow block (persist the block on the attempt row at dispatch — simplest is a JSON column travel with the existing attempt insert; check what `create_attempt` already stores with `rg -n "def create_attempt" src/nightshift/manager/store.py`): `route_to = workflows.route(step, body.signal)`, `dest_visits_exhausted` from `parse_visits(meta)`, etc. Enforce the 256 KB `document` cap here (over-cap → coerce the outcome to WORKER_ERROR before transitioning).
...
- UI: create-panel segment **Off | Enhance | Workflow** + definition picker (populate from a new `GET /api/workflows` returning names — add the trivial endpoint in `api_operator.py`) + optional planner-model input; queue-row step badge from `workflow_step`/`workflow_visits`; read-only artifact viewer in the detail pane (fetch via a `GET /api/tasks/<id>/artifacts` endpoint reading `read_artifacts`). Manual check with a live manager; keep JS consistent with the existing segmented-control pattern.
