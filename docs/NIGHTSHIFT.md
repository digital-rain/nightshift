...
| `model` | string | `auto` (config `default_model`) | Worker-interpreted model. `auto` lets the worker pick the most cost-effective capable model; `max` uses its most capable configuration; an explicit id (e.g. `gemini-3-pro`) routes to **any worker that advertises that id** (capability-based routing — no vendor→backend prefix map) and must be in config.json `scheduled_models` (a pin-only allow-set). A worker can remap an explicit id via its `model_aliases`. Leave unset unless the task needs a specific capability. In the GitHub Actions path an explicit model outside `scheduled_models` makes the task **dispatch-only** (nightly skips it; runs only via `workflow_dispatch` with `task=<name>` as a cost guard). |
