"""Launch the Nightshift UI server.
    ...
    python -m nightshift.server [--root .] [--port 8799] [--host 127.0.0.1]

Port resolution order: --port flag, then NIGHTSHIFT_UI_PORT, then the player
settings file, then the default (8799). Avoids 8765, which the Cursor IDE binds.
"""
...
def resolve_port(root: Path, cli_port: int | None) -> int:
    ...
    if env_port and env_port.isdigit():
        return int(env_port)
    value = load_settings(root).get("port", DEFAULTS["port"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    port = resolve_port(root, args.port)
    app = create_app(root)
    print(f"Nightshift UI → http://{args.host}:{port}  (root: {root})")
